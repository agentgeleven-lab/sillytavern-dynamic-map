...
export function createPanel(store,persistence,preferences,options={}){
    ...
    function renderEditor(map){
        ...
        if(editorTool==='route'){
            ...
            const displayLength=field(form,'图上目标长度（80–2000，留空自动）',input(edge?.metadata.displayLength??'','number'));displayLength.min='80';displayLength.max='2000';displayLength.step='any';displayLength.disabled=isTileMap(map);form.append(el('p',isTileMap(map)?'格子地图的线长由两端格子位置决定，请拖动地点调整。':'图上长度不影响实际距离或通行时间。自动布局尽量满足目标长度；拥挤、闭环或固定地点可能使结果偏离，其他地点也可能重新排列。','dm-help'));
