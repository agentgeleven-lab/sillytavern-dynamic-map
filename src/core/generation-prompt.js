...
需要表达长路或岔路时可添加 waypoint 途经点，但仅当目录存在该类型；途经点仍包含全部 Node 字段；纯示意节点在 description 标明“路线示意途经点”，真实交通设施则说明实际用途和素材依据，不得冒充真实聚落。
远距离交通节点：当素材或用户要求明确说明两个地点相距遥远，且世界设定支持连接它们的长途交通方式时，优先表达为“地点 A → A 侧交通节点 → B 侧交通节点 → 地点 B”。这里的箭头仅表示连接顺序，不表示单向，bidirectional 仍按实际通行规则填写。不得根据图上位置或像素线长判断距离遥远，不设跨世界通用的公里阈值。
交通节点使用已有 waypoint（途经点）类型，不新增 station 类型。按世界背景选用车站、码头、驿站、空港或传送节点，不在不支持铁路的世界凭空生成火车站，不凭空发明交通技术。优先复用素材或当前地图中已有的交通节点和 ID；同一枢纽可以连接多个远方地点，不为每条道路重复生成车站。素材仅支持普通长路时保留普通道路或示意途经点，不强行增加成对车站；目录没有 waypoint 时跳过自动补充交通节点，不改类型目录。
两端交通设施已有名称时采用原名；只有长途交通关系明确但站点名称未给出时，才用“A 交通站点”等中性名称补充示意，并在 description 明确标注“路线示意途经点，推定”，不伪称真实站名。交通节点名称属于地点 name，不受道路命名开关限制；道路 name 和 distance 仍遵守本次开关。三段连接均只能引用实际生成或已有的地点 ID，道路类型选择目录中最匹配的类型，在 metadata.note 说明长途段与本地接驳段的用途，不自动添加新道路类型。
只在本次地图范围与层级适合表达跨城或跨区域交通时使用此结构，不给建筑内部的房间或普通相邻地点增加长途站点，不新建其他层级。交通节点计入总节点预算；预算不足时优先保留重要真实地点和完整连接，不输出悬空道路。新增模式保留既有直达道路，不擅自拆改；只补充缺少且有依据的交通节点和连接。
长途站间距离与全程距离必须区分：只知道站间距离时，不把它当成含两端接驳的全程距离；只知道全程距离却无法确定接驳长度时，不把全程距离重复填到每段，可保留各段 null 并在 metadata.note 记录已知全程。新增示意车站不能增加实际总里程；没有依据的接驳距离不要编造为 0。
