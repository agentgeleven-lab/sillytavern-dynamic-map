...
以用户要求、角色卡和本次提供的世界书为主要依据，不假设已经读取未提供的聊天记录。对于现实地点，允许依据模型自身的地理与交通世界知识判断远近，补全真实车站、线路、必要换乘与接驳关系，不要求这些信息先出现在素材中；明确的时代背景、架空改动与用户设定优先。此任务不联网，不得声称搜索、实时查询或核实过资料。地点同名且明确为同一地点时合并，别名写入 ai.alias；不同地区的同名地点不得直接合并。
...
需要表达长路或岔路时可添加 waypoint 途经点，但仅当目录存在该类型；途经点仍包含全部 Node 字段；纯示意节点在 description 标明“路线示意途经点”，真实交通设施则说明实际用途及素材依据或模型知识依据，不得冒充真实聚落。
远距离交通网络：对于现实地点，即使素材没有说明距离或列出车站，也应依据模型自身世界知识判断地理跨度和通常出行方式；相距较远且通常通过公共交通往来时，主动补全关联的真实车站与连接，不以“资料未出现”为由省略。结构为“地点 A — 出发站 — 必要换乘站或分支站 — 到达站 — 地点 B”，并非固定只有两站或三段路。连接顺序不代表单向，bidirectional 按实际通行规则填写。不得根据图上像素线长判断远近，不设置跨世界通用的公里阈值。
交通节点使用目录已有的 waypoint（途经点）类型，不新增 station 类型；目录没有 waypoint 时不修改目录，在相关地点 description 提示缺少途经点类型。按时代与世界背景选择铁路站、地铁站、码头、驿站等，不凭空增加不符合设定的交通技术。现实地图使用模型知道的真实站名与合理线路、换乘关系，不生成“A 交通站点”之类占位车站，不因学校或设施与车站名称相似就认定相邻。
保留起终点站、必要换乘站，以及能前往其他目标地点的分支站。普通中间站可以省略，但在区间道路 metadata.note 说明“省略普通中间站”；省略不代表两站实际相邻，不能省略必要换乘后伪称直达。不同目的地共用同一车站 ID，已有车站复用，不为每条路线生成重复节点。分支示意为“住所 — 车站甲 — 车站乙 — 车站丙 — 学校”，同时“车站乙 — 店铺”；此例仅表示拓扑，不是具体地理事实。没有私人住所或店铺的精确地址时可按已知区域推定接驳并标注，不能捏造地址或精确步行距离。
依靠模型世界知识补全的节点在 description 标明“依据模型世界知识，未经联网核实”；连接在 metadata.note 标明同样依据。对拿不准的线路或换乘关系标注“交通关系待确认”，合理推定的连接同时标注“推定”；完全无法合理判断的部分保留缺口，不虚构站名或强行补线。虚构世界仍依据设定合理补充示意交通节点，并明确标注推定，不冒充现实设施。
交通节点的地点 name 不受道路命名开关限制；道路 name 和 distance 仍遵守本次开关。各段只引用本图中实际生成或已有的节点 ID，道路类型从目录选择最匹配项，在 metadata.note 区分铁路等交通区间与步行接驳，不新增道路类型。不改变当前生成范围或创建其他层级，不给建筑内部房间或普通相邻地点强加长途车站。途经点计入总节点预算，优先保留目标地点、必要换乘及分支站，通过省略普通中间站控制数量，不为满足少节点目标伪造直达关系。
新增模式保留既有道路，只补缺失节点与连接，不擅自拆改原来的直达道路；模型世界知识也是允许的补全依据。复用已有枢纽，避免重复节点、重复道路及悬空引用。
长途站间距离与全程距离必须区分：只知道站间距离时，不把它当成含两端接驳的全程距离；只知道全程距离却无法确定接驳长度时，不把全程距离重复填到每段，可保留各段 null 并在 metadata.note 记录已知全程。补全交通节点不能增加实际总里程；没有依据的接驳距离不要编造为 0。
