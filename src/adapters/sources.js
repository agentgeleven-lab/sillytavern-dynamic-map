...
export async function collectMapSources(ctx, wi, {includeGlobal=false, guard=()=>{}, onProgress=()=>{}}={}) {
    guard();
    ...
    let loaded=0;for(const name of names){guard();onProgress(`读取世界书 ${++loaded}/${names.size}：${name}`);if(typeof wi.loadWorldInfo!=='function')throw new Error('酒馆缺少世界书读取接口');const book=await wi.loadWorldInfo(name);guard();if(!book?.entries)throw new Error(`世界书「${name}」读取失败，未开始生成`);source.世界书.push({名称:name,条目:entries(book.entries)});}
    if(!cd.extensions?.world&&cd.character_book?.entries){source.世界书.push({名称:cd.character_book.name||'角色卡内嵌世界书',条目:entries(cd.character_book.entries)});}
