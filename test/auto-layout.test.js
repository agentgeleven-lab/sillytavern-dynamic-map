...
test('automatic layout retains different existing road scales and ignores obsolete targets',()=>{
 const m=make(3,[[0,1],[1,2]]);m.nodes.n0.position={x:0,y:0};m.nodes.n1.position={x:200,y:0};m.nodes.n2.position={x:1000,y:0};m.edges[1].metadata.displayLength=160;
 const distances=m.edges.map(e=>e.distance);autoLayout(m);const length=e=>Math.hypot(m.nodes[e.from].position.x-m.nodes[e.to].position.x,m.nodes[e.from].position.y-m.nodes[e.to].position.y);
 assert.ok(length(m.edges[1])>length(m.edges[0])*2.5);assert.deepEqual(m.edges.map(e=>e.distance),distances);const before=structuredClone(m);layoutMap(m);assert.deepEqual(m,before);
});
